...
const RoomPage = () => {
	...
	return (
		<div className='w-full h-screen flex flex-col p-2 gap-2'>
			...
			<div className='font-bold text-center p-5 border rounded'>
				房间号 {id}
			</div>
			<div className='w-full flex flex-row flex-1 overflow-auto gap-2'>
				<div className='flex-1 overflow-auto p-5 grid grid-cols-4 gap-5 rounded border'>
					{participants &&
						Object.keys(participants).map((participant: string) => {
							return (
								<div key={participant} className='p-2 border rounded-lg h-min'>
									{participants[participant].userName}
								</div>
							);
						})}
				</div>
				<div className='w-[30%] overflow-auto p-5 flex flex-col space-y-2 rounded border'>
					<div
